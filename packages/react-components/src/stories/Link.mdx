...
You can configure a link's behaviour using the `href` to provide a destination URL, or use the `onPress` prop to configure click/press events.
...
Use the `size` prop to choose between `small` (default) and `medium` sizes:
...
**Note**: when using an icon-only link, you must also pass an `aria-label` or `aria-labelledby` prop, to provide an accessible label to assistive technologies.
