...
Text size and styling align to the [B.C. Design System typescale](https://www2.gov.bc.ca/gov/content?id=72C2CD6E05494C84B9A072DD9C6A5342). If no `size` prop is passed, it will default to `medium`.

Links will also respect generic text sizing like bold or italics. They will match the correct heading style from the design system typescale if enclosed in an element like `<h2>`:

<Canvas of={LinkStories.LinkInHeading} />

### Link icons

The link component supports optional icon slots to the left and right of the link label:

<Canvas of={LinkStories.LinkWithLeftIcon} />
<Canvas of={LinkStories.LinkWithRightIcon} />

You can pass an icon to one of these slots and leave the `children` slot empty to create an icon-only link:
